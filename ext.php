...
class ext extends base
{
	...
	public function is_enableable()
	{
		...
		$user->add_lang_ext('avathar/bbguildeq2', 'info_ext');

		if (version_compare(PHP_VERSION, self::MIN_PHP_VERSION, '<'))
		{
			$errors[] = $user->lang('BBGUILDEQ2_PHP_VERSION_FAIL', self::MIN_PHP_VERSION, PHP_VERSION);
		}

		if (phpbb_version_compare(PHPBB_VERSION, self::MIN_PHPBB_VERSION, '<'))
		{
			$errors[] = $user->lang('BBGUILDEQ2_PHPBB_VERSION_FAIL', self::MIN_PHPBB_VERSION, PHPBB_VERSION);
		}

		$ext_manager = $this->container->get('ext.manager');
		if (!$ext_manager->is_enabled('avathar/bbguild'))
		{
			$errors[] = $user->lang('BBGUILDEQ2_REQUIRES_BBGUILD');
		}
		else
		{
			...
			{
				$errors[] = $user->lang('BBGUILDEQ2_REQUIRES_BBGUILD_VERSION', self::MIN_BBGUILD_VERSION, $core_version);
			}
		}
