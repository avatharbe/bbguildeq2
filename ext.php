...
{
	const BBGUILDEQ2_VERSION = '2.0.0-rc2';
	...
	public function is_enableable()
	{
		...
		$language->add_lang('info_ext', 'avathar/bbguildeq2');

		if (version_compare(PHP_VERSION, self::MIN_PHP_VERSION, '<'))
		{
			$errors[] = $language->lang('BBGUILDEQ2_PHP_VERSION_FAIL', self::MIN_PHP_VERSION, PHP_VERSION);
		}

		if (phpbb_version_compare(PHPBB_VERSION, self::MIN_PHPBB_VERSION, '<'))
		{
			$errors[] = $language->lang('BBGUILDEQ2_PHPBB_VERSION_FAIL', self::MIN_PHPBB_VERSION, PHPBB_VERSION);
		}

		$ext_manager = $this->container->get('ext.manager');
		if (!$ext_manager->is_enabled('avathar/bbguild'))
		{
			$errors[] = $language->lang('BBGUILDEQ2_REQUIRES_BBGUILD');
		}
		else
		{
			...
			{
				$errors[] = $language->lang('BBGUILDEQ2_REQUIRES_BBGUILD_VERSION', self::MIN_BBGUILD_VERSION, $core_version);
			}
		}
